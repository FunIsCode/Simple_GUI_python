    ...
    def throw(self):
        ...
        for i in range(DICE_NUMBER):
            while True:
                ...
                good_number = dice_number != self.__current_dice_score[i]
                if good_number:
                    break
            self.__current_dice_score[i] = dice_number

        print(self.__current_dice_score)
        for i in range(DICE_NUMBER):
            label = self.__dicepiclabels[i]
            label.configure(image=self.__dicepics[self.__current_dice_score[i]])
